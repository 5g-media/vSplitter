...
# docker rmi -f splitter
# docker build -t splitter .
# docker run --name splitter -p 9998:9998/udp --rm splitter
# docker run --name splitter -e DEBUG=true -p 9998:9998/udp --rm splitter
...
RUN \
	    DIR=/tmp/srt && \
        git clone --branch ${SRT_VERSION} --depth 1 https://github.com/Haivision/srt.git ${DIR} ; \
        cd ${DIR} ; \
